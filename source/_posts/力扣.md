...
层次遍历和上这不同，是通过广度优先遍历实现，需要扫描完该层的数据，才对下一层数据进行查询。
...
3. **BFS** 循环： 当队列 queue 为空时跳出。
   ...
   2. ​	当前层打印循环： 循环次数为当前层节点数（即队列 queue 长度）。
      1. ​	出队： 队首元素出队，记为 `cur`。
      2. ​	打印： 将 `cur.val` 添加至 `level` 尾部。
      3. 添加子节点： 若 cur 的左（右）子节点不为空，则将左（右）子节点加入队列 queue 。
...
class Solution {
    ...
    public boolean isbalance(TreeNode left, TreeNode right){
        ...
        boolean a = isbalance(left.left, right.right);
        boolean b = isbalance(left.right, right.left);
        return a && b;
    }
...
递推工作：
初始化节点 `tmp` ，用于交换 `root` 的左右节点。
开启递归 左子节点 `invertTree(root.left)` ，并将返回值作为 root 的 左子节点 。
开启递归 右子节点 `invertTree(root.right)` ，并将返回值作为 root 的 右子节点 。
返回值： 返回当前节点 root 。
...
![Picture1.png](1629825510-roByLr-Picture1.png)

因此递归参数为：根节点在前序遍历的索引 `root` 、子树在中序遍历的左边界 `left` 、子树在中序遍历的右边界 `right` 。

**终止条件：** 当 `left > right` ，代表已经越过叶节点，此时返回 *n**u**ll* 。
