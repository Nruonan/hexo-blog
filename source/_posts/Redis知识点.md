...
- 哈希冲突的解决方式：
  ...
  - 渐进式 rehash：
    ...
    - 把一次性大量数据迁移工作的开销，分摊到了**多次**处理请求的过程中，避免了**一次性** rehash 的耗时操作
...
3. Set：底层结构为 IntSet、HashTable，应用场景为 白名单、共同关注，抽奖等
   - 如果集合中的元素都是整数且元素个数小于 512 （默认值，set-maxintset-entries配置）个，Redis 会使用**整数集合**作为 Set 类型的底层数据结构；
   - 如果集合中的元素不满足上面条件，则 Redis 使用**哈希表**作为 Set 类型的底层数据结构。

4. Hash：底层结构为 ZipList、HashTable，应用场景为 缓存对象等
   - 如果哈希类型元素个数小于 512 个（默认值，可由 hash-max-ziplist-entries 配置），所有值小于 64 字节（默认值，可由 hash-max-ziplist-value 配置）的话，Redis 会使用**压缩列表**作为 Hash 类型的底层数据结构；
   - 如果哈希类型元素不满足上面条件，Redis 会使用**哈希表**作为 Hash 类型的 底层数据结构。

5. Zset：底层结构为 ZipList、ZSkipList，应用场景为 用户活跃度排行榜，电话排序
...
<img src="assets/image-20240916110319946.png" alt="image-20240916110319946"  />
